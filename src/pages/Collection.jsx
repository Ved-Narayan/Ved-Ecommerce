...
const Collection = () => {
  ...
  //get the data of all the items using the context API
  const {products , search , showSearch} = useContext(ShopContext);
  //filter state variable
  const [showFilter,setShowFilter] = useState(false);
  //state variable for mapping the products
  const [filterProducts,setFilterProducts] = useState([]);


  //for assingning the data to the filters
  const [category,setCategory] = useState([]);
  const [subCategory,setSubCategory] = useState([]);

  //for assigning sort in prices
  ...
  const applyFilter = () => {
    ...
    let productsCopy =  products.slice();4
